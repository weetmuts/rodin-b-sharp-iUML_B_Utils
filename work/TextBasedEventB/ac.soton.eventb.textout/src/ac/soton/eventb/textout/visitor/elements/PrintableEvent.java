...
public class PrintableEvent implements IPrintable {
	...
	public List<String> print() {
		...
		output.add("event " + event.getName() + internalString + extendedString + convergenceString + " " + comment);
		...
		if (guardList.size() > 0) {
			...
			for (Guard guard : guardList) {
				List<String> guardStringList = new PrintableGuard(guard).print();
		...
		if(actionList.size()>0){
			...
			for(Action action: actionList){
				List<String> actionStringList = new PrintableAction(action).print();
