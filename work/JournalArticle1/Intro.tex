...
Initial development of the code generation approach began from an object-oriented perspective, with the implementation-level notation, Object-Oriented, Concurrent B (OC-B)~\cite{AEThesis}. The focus, here, was to apply Event-B  modelling, with the OC-B implementation-level notation, to facilitate the modelling and implementation of Object-Oriented concurrent systems, initially targeting Java. We found, however, that this approach gave rise to a notation, where the semantic gap between Event-B and the OC-B was quite large. This was not optimal for developers of Event-B models, who then wanted to write implementation-level specifications, from which code could be generated. For this reason the OC-B approach was distilled, and gave rise to Tasking Event-B. 

We found that the Ada programming language provide clearly defined constructs, that map well to Event-B. 



The target languages, for code-generation, as largely determined 
